...
- Name: dashboard_stats.spec.js >> Given stats API fails, Then ApplicationTable is still rendered
- Location: e2e/dashboard_stats.spec.js:151:1
...
Locator: locator('.application-table, [data-testid="app-table"]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.application-table, [data-testid="app-table"]').first()
...
- banner:
  ...
  - navigation:
    - link "⚙ Settings":
      - /url: /settings
- text: ⚠ Could not load dashboard stats — HTTP 500
- button "Retry"
- text: Total Applications — Active In Pipeline — pending / gate-passed / gate-unknown Workspace Users — LLM Success Rate — primary backend calls Application Status No applications yet Job Execution Last 24 hours No activity in the last 24 h LLM Resilience Primary vs fallback — last 7 days No LLM calls recorded in the last 7 days
- main:
  - heading "Applications 1" [level=2]
  - table:
    - rowgroup:
      - row "Select all applications Candidate Job Status Score Created":
        - columnheader "Select all applications":
        ...
        - columnheader
    - rowgroup:
      - row "Select Alice Chen Alice Chen alice@example.com Senior Data Engineer Scored 82% Jan 11, 2024 Review Alice Chen":
        - cell "Select Alice Chen":
          - checkbox "Select Alice Chen"
        - cell "Alice Chen alice@example.com"
        - cell "Senior Data Engineer"
        - cell "Scored"
        - cell "82%"
        - cell "Jan 11, 2024"
        - cell "Review Alice Chen":
          - link "Review Alice Chen":
            - /url: /review/app-1
            - text: Review →
```
...
```ts
  55  | // ── Fixtures ──────────────────────────────────────────────────────────────────
  56  | 
  57  | const APPLICATIONS = [
  58  |   {
  59  |     id:                        'app-1',
  60  |     candidate_name:            'Alice Chen',
  61  |     candidate_email:           'alice@example.com',
  62  |     job_title:                 'Senior Data Engineer',
  63  |     status:                    'scored',
  64  |     final_score:               0.82,
  65  |     is_evaluated_via_fallback: false,
  66  |     created_at:                '2024-01-12T00:00:00Z',
  67  |     updated_at:                '2024-01-15T00:00:00Z',
  68  |   },
  69  | ];
  70  | 
  71  | function makeStats({
  72  |   applications     = 142,
  73  |   active_jobs      = 18,
  74  |   workspace_users  = 9,
  75  |   llm_success_rate = 94.2,
  76  |   statusDist       = [],
  77  |   funnelData       = [],
  78  |   timeSeries       = [],
  79  | } = {}) {
  80  |   return {
  81  |     totals: { applications, active_jobs, workspace_users, llm_success_rate },
  82  |     application_status_distribution: statusDist,
  83  |     job_execution_funnel: funnelData,
